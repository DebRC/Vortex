...
NUM_PROOFS = int(os.getenv("GAS_LIMIT"))//PER_TXN_GAS
...
def generate_results(start_block, end_block):
    ...
    with open(os.path.join(READINGS_DIR, "block_details.csv"), "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["blockNumber", "gasUsed", "baseFeePerGas(wei)", "baseFeeSpike(mwei)", "txCount"]
        )
        writer.writeheader()
    ...
    print(f"Gas Used per Block: {gas_used_per_block}")
...
def main():
    signed_txns = sign_proofs(NUM_ROUNDS*NUM_PROOFS)
    start_block = w3.eth.block_number
    start_cpu_profiling()
    for i in range(0,len(signed_txns), NUM_PROOFS):
        signed_txns_batch = signed_txns[i:i+NUM_PROOFS]
        tx_hash_list = submit_proofs(signed_txns_batch)
